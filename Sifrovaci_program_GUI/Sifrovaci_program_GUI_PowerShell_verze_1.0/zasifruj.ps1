...
if ($delka_nazvu_jednoho_adresare -lt 74) {
#echo "pridat"
...
echo "vybran soubor klice $nazev_klice"
sleep 3
...
######################################################
# zde vytvoreni souboru file comment
######################################################
...
$zip_file_comment=@() # muzi bejt typ pole, jinak dava radky za sebe , mito pod sebe
$zip_file_comment+=$reference_na_heslo
$zip_file_comment+=$sel_file_key
$zip_file_comment+=$zip_date
Set-Content -Path "$path$filecomment_name" -Encoding ASCII -Value $zip_file_comment
sleep 1

# test jestli by sprvne vytvorem soubor file comment
...
if ( $d_kontrola_file_comment -ne 3 ){
...
Remove-Item -Path "$path$file_bat" -Force -ErrorAction SilentlyContinue
sleep 1
