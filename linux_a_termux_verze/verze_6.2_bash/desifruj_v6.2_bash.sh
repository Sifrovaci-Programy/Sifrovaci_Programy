...
ls $cesta_soubory_klicu$read_zip_file_comment_soubor_hesel 1>/dev/null 2>/dev/null
if [ "$?" != "0" ]; then
...
sha=$(sha1sum $cesta_soubory_klicu$read_zip_file_comment_soubor_hesel)
sha2=${sha:0:40}
echo $sha2
...
soubor_hesel_2=$cesta_soubory_klicu$read_zip_file_comment_soubor_hesel
